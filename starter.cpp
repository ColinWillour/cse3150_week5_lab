...
std::vector<int> compress_row(const std::vector<int>& row) {
	...
	std::vector<int> compressed; // creates a vector compressed
	std::copy_if(
		row.begin(), row.end(), // start at row.begin(), stop at row.end()
		std::back_inserter(compressed), // so i dont have to track positions
		[](int x) {return x != 0;} // returns true if x is not 0 and false if it is
	); 
	while (compressed.size() < 4) {
		compressed.push_back(0); // adds 0's if there aren't 4 lines
	}
	return compressed;
}
...
bool move_right(std::vector<std::vector<int>>& board) {
	...
	for (int i = 0; i <  4, i++) {
		std::vector<int> reversed(board[i].rbegin(), board[i].rend());
		std::vector<int> row;
		row = compress_row(reversed);
		row = merge_row(row);
		if (row != board[i]) {
			moved = true;
		}
...
bool move_up(std::vector<std::vector<int>>& board) {
	...
	for (int j = 0; j < 4; j++) {
		...
		std::vector<int> orig_col = col;
		std::vector<int> finish_col;
		finish_col = compress_row(col);
		finish_col = merge_row(col);
		for (int i = 0; i < 4; i++) {
			board[i][j] = finish_col;
		}
		if (orig_col != board[j]) {
			moved = true;
		}
